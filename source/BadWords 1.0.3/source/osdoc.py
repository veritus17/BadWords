...
class OSDoctor:
    ...
    def check_dependencies(self):
        ...
        Updated to support pipx/system-wide whisper binary check.
        """
        missing = []
        ...
        # 1. First check for Executable in PATH (Supports pipx / Linux System install)
        if shutil.which("whisper"):
            whisper_found = True
        elif self.is_win and shutil.which("whisper.exe"):
            whisper_found = True
            
        # 2. Fallback: Check for Python Module (Supports Windows legacy / portable envs)
        ...
        if not whisper_found:
            missing.append("openai-whisper (command or module)")
