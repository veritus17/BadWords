...
class AudioEngine:
    ...
    def start_timeline_generation(self, words_data, settings, callbacks):
        ...
        def runner():
            success = self.assemble_timeline(
                words_data,
                settings,
            ...
            if success:
                if callbacks.get('on_success'): callbacks['on_success']()
            else:
    ...
    def assemble_timeline(self, words_data, settings, callback_status=None, callback_progress=None):
        ...
        try:
            ...
            # 2. HANDLE COMPOUND CLIP FIX
            if settings.get("compound", False):
                ...
                # Standard Mode: Get V1 Clip
                source_item = self.resolve_handler.get_current_source_item()
                if not source_item:
                    log_error("Could not find source clip on V1.")
                    return False
